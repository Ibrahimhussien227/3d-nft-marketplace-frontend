...
const MenuItems = ({ isMobile, active, setActive, setIsOpen }) => {
  const { setAnotherAccount, currentAccount } = useContext(NFTContext);

  const { names } = getUserProfile(currentAccount);

  const generateLink = (i, address) => {
    switch (i) {
      case 0:
        return "/";
      case 1:
        return "/listed-nfts";
      case 2:
        return names === "No Name" || names.length === 0
          ? `/collection?my-nft=${address}`
          : `/collection?my-nft=${names.name}`;
      default:
        return "/";
    }
  };
  ...
  return (
    ...
    >
      {["Explore NFTs", "Listed NFTs", "My NFTs"].map((item, i) => (
        ...
        >
          <Link
            href={generateLink(i, currentAccount)}
            onClick={() => {
              localStorage.setItem("anotherAccount", currentAccount);
              setAnotherAccount(currentAccount);
            }}
          >
            {item}
          </Link>
